...
class ETLManagerTests(ArchesTestCase):
    ...
    def test_node_error_view(self):
        error = LoadErrors.objects.create(
            load_event=self.admin_load_event, message="Early failure."
        )

        self.client.force_login(self.full_name_user)
        response = self.client.get(
            reverse("etl_manager"),
            QUERY_STRING=f"action=nodeError&loadid={self.admin_load_event.pk}&nodeid=null&error=null",
        )
        ...
        response = self.client.get(
            reverse("etl_manager"),
            QUERY_STRING=f"action=nodeError&loadid={self.admin_load_event.pk}&nodeid=null&error={error.error}",
        )
        ...
        response = self.client.get(
            reverse("etl_manager"),
            QUERY_STRING=f"action=nodeError&loadid={self.admin_load_event.pk}&nodeid={error.node_id}&error={error.error}",
        )
