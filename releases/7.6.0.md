...
19. In the same terminal window where you ran `npm` ( on the same level as `package.json` ):
    ...
    2.  Run `python manage.py start_webpack_development_server` or `python manage.py build_development`. This will generate your `media/build` directory.
        - If running your project in development:
            - `python manage.py start_webpack_development_server` will build the frontend of the application and then start a webpack development server
            - `python manage.py build_development` will build a development bundle for the frontend assests of the application and call `collectstatic` -- this should complete in less than 2 minutes
        - If running your project in production:
            - `python manage.py build_production`. This will create a production bundle of frontend assets and also call `collectstatic` -- **this make take up to 2 hours depending on system resources**
